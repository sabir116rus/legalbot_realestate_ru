...
1. Добавь в репозиторий файлы `Procfile` и `runtime.txt` (уже присутствуют в этой версии проекта). Первый запускает две процессы: HTTP‑сервис с политикой конфиденциальности (`web`) и сам Telegram‑бот (`worker`). Второй фиксирует версию Python `3.11.9`, совместимую с buildpack.
